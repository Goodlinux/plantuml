...
RUN echo "apk -U upgrade "						> /usr/local/bin/UpdtPlantuml  \
    ...
    && echo "sleep 15"					>> /usr/local/bin/entrypoint.sh  \
    && echo "echo 'Modification de footer'"					>> /usr/local/bin/entrypoint.sh  \
    && echo "rm /app/target/plantuml/resource/footer.jsp"					>> /usr/local/bin/entrypoint.sh  \
